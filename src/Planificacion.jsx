    ...
    return (
        <div className="pl-[52px] mt-3 relative animate-in fade-in slide-in-from-top-2 duration-300">
            <div className={`w-full p-2.5 border rounded-lg shadow-sm flex items-center justify-between text-xs font-bold ${bgClass}`}>
                ...
                {/* Renderizado Condicional del ETA */}
                {showEta && eta && (
                    <span className="flex items-center gap-1.5 text-orange-600"><Zap className="w-4 h-4 text-orange-500"/> LLEGADA: {eta}</span>
                )}
            </div>
        </div>
    );
...
export default function Planificacion() {
  ...
  // Variable para controlar si mostramos o no la hora de llegada
  ...
  const handlePassengerSelectForPoint = (pointType, waypointIndex, selectedPassengerName) => {
    if (!selectedPassengerName) {
        ...
        if (pointType === 'waypoint') {
            const w = [...waypoints];
            w[waypointIndex] = { address: '', lat: null, lng: null, contact: '', passengerName: '' };
            setWaypoints(w);
        }
        return;
    }
    ...
    const newPointData = { 
        address: personLocation.address, 
        lat: parseFloat(personLocation.lat), 
        lng: parseFloat(personLocation.lon || personLocation.lng), 
        contact: selectedPassengerName, 
    ...
    if (pointType === 'start') setStartPoint(newPointData);
        ...
        updatedWaypoints[waypointIndex] = newPointData;
        setWaypoints(updatedWaypoints);
    }
  };

  // --- ALGORITMO PARA OPTIMIZAR EL ORDEN DE LOS PUNTOS INTERMEDIOS ---
  const handleOptimizeRoute = () => {
      if (!startPoint?.lat) {
          return alert("Debes definir el 'Punto de Inicio' primero para poder optimizar la ruta.");
      }
      ...
      if (validWaypoints.length < 2) {
          return alert("Necesitas al menos 2 paradas intermedias para poder optimizarlas.");
      }
      ...
      // Función para calcular distancia recta (Haversine)
      ...
      // Greedy Algorithm (Vecino más cercano)
      while (unvisited.length > 0) {
          ...
          // Agregamos el más cercano y lo quitamos de los pendientes
          currentLoc = unvisited[nearestIdx];
          optimizedWaypoints.push(unvisited[nearestIdx]);
          unvisited.splice(nearestIdx, 1);
      }

      // Actualizamos los waypoints con el nuevo orden, dejando al final los que estaban vacíos
  ...
  const handleSaveRoute = async () => {
      if(!newRoute.client || !startPoint?.address || !endPoint?.address || !newRoute.driver) return alert("Faltan datos obligatorios (Empresa, Origen, Destino, Conductor).");
      ...
      <div className="flex-1 flex gap-6 overflow-hidden">
          ...
          <div className="w-1/3 flex flex-col gap-4 overflow-y-auto pr-2 pb-4 scrollbar-thin">
              {routesList.map((ruta) => (
                <div key={ruta.id} onClick={() => setViewRoute(ruta)} className={`bg-white p-4 rounded-xl shadow-sm border transition cursor-pointer group ${viewRoute?.id === ruta.id ? 'border-blue-500 ring-1 ring-blue-500 shadow-md' : 'border-slate-200 hover:shadow-md'}`}>
                    ...
                    {/* ETIQUETAS DE PASAJEROS QUE VIAJAN */}
                    {(() => {
                        const passengers = [
                            ruta.startCoords?.contact,
                            ...(ruta.waypointsData?.map(w => w.contact) || []),
                            ruta.endCoords?.contact
                        ].filter(Boolean);
                    ...
                    <div className="space-y-2 mt-3">
                        ...
                        {ruta.waypointsData && ruta.waypointsData.length > 0 && (
                            <div className="pl-5">
                                <div className="text-[10px] text-slate-500 font-bold bg-slate-50 border border-slate-100 rounded px-2 py-1 inline-flex items-center gap-1">
                                    <Users className="w-3 h-3"/> {ruta.waypointsData.length} paradas intermedias
                                </div>
                            </div>
                        )}
          ...
          <div className="flex-1 bg-slate-200 rounded-xl border border-slate-300 relative overflow-hidden flex items-center justify-center shadow-inner">
             ...
             {viewRoute?.technicalData && viewRoute.technicalData.totalDistance > 0 && (
                <div className="absolute bottom-4 right-4 bg-white/90 backdrop-blur-sm p-4 rounded-xl shadow-xl border border-slate-200 max-w-xs z-[10] animate-in fade-in slide-in-from-bottom-4 duration-500">
                     ...
                     <div className="flex justify-between text-sm mb-1 gap-4">
                        ...
                        <b className="text-slate-900">{viewRoute.technicalData.totalDistance} km</b>
                     </div>
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            <div className="bg-white w-full max-w-6xl h-[95vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden">
                    ...
                    <div className="w-[45%] p-6 overflow-y-auto border-r border-slate-100 bg-white z-10 shadow-[5px_0_15px_-5px_rgba(0,0,0,0.1)] relative scrollbar-thin">
                        <div className="space-y-6">
                            ...
                            <div className="relative pt-2 pb-6">
                                ...
                                {/* LÍNEA CONECTORA DINÁMICA DE FONDO */}
                                ...
                                <div className="bg-slate-50/50 p-4 rounded-xl border border-slate-200 mb-6 mt-3 relative shadow-sm z-10">
                                    ...
                                    {/* CAJA DE RESUMEN INCRUSTADA EN EL DESTINO */}
                ...
                <div className="p-4 border-t border-slate-200 flex justify-end gap-3 shrink-0 bg-white">
                    ...
                    <button onClick={handleSaveRoute} className="px-6 py-2.5 text-sm font-black text-white bg-blue-600 rounded-lg hover:bg-blue-700 shadow-lg shadow-blue-600/30 flex items-center gap-2 transition"><Navigation className="w-4 h-4"/> Asignar y Confirmar Ruta</button>
                </div>
            </div>
        </div>
      )}
