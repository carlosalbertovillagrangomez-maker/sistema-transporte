...
function App() {
  ...
  // --- NUEVO: ESTADOS DEL CENTRO DE COMUNICACIÓN ---
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
        ...
        const alerts = routesArr.filter(r => r.proximityAlert?.active === true);
        ...
        // Actualizar el modal de chat en tiempo real si está abierto
        setChatModalRoute(prevChat => {
            if (!prevChat) return prevChat;
            const updated = routesArr.find(r => r.id === prevChat.id);
            return updated || prevChat;
        });
  ...
  // Auto-scroll del chat del Despachador
  useEffect(() => {
  ...
  // --- NUEVO: FUNCIÓN PARA ENVIAR MENSAJE COMO DESPACHO ---
  const sendDispatchMessage = async () => {
      ...
      try {
          ...
          setChatInput('');
      } catch(e) { console.error("Error al enviar mensaje desde despacho", e); }
  };
        ...
        <header className="h-16 bg-white border-b flex items-center justify-between px-8 shadow-sm z-10 shrink-0">
          ...
          <div className="flex items-center gap-6">
              <div className="relative cursor-pointer">
                  ...
                  {activeAlertsCount > 0 && (
                      <span className="absolute -top-1 -right-1 h-4 w-4 bg-red-500 text-white text-[10px] font-bold flex items-center justify-center rounded-full border-2 border-white animate-pulse">{activeAlertsCount}</span>
                  )}
              </div>
            ...
            <div className="flex-1 flex overflow-hidden p-6 gap-6 animate-[fadeIn_0.3s_ease-out]">
                ...
                <div className="flex-1 relative bg-slate-200 rounded-xl shadow-inner overflow-hidden border border-slate-300">
                            ...
                            {selectedRoute && selectedRoute.technicalData?.geometry?.length > 0 && (
                                <>
                                    ...
                                    {selectedRoute.currentLocation && selectedRoute.status === 'En Ruta' && (
                                        <Marker position={selectedRoute.currentLocation} icon={{ path: window.google.maps.SymbolPath.CIRCLE, scale: 10, fillColor: "#22c55e", fillOpacity: 1, strokeWeight: 3, strokeColor: "white" }} zIndex={999}/>
                                    )}
                                </>
                            )}
                        </GoogleMap>
                    ...
                    {selectedRoute?.status === 'En Ruta' && selectedRoute?.currentLocation && (
                        <div className={`absolute top-4 right-4 bg-white/90 backdrop-blur px-3 py-1.5 rounded-full shadow-md z-[500] border flex items-center gap-2 ${selectedRoute.proximityAlert?.active ? 'border-orange-300' : 'border-green-200'}`}>
                            <div className={`w-2.5 h-2.5 rounded-full animate-pulse ${selectedRoute.proximityAlert?.active ? 'bg-orange-500' : 'bg-green-500'}`}></div>
                            <p className={`text-[10px] font-black uppercase tracking-widest ${selectedRoute.proximityAlert?.active ? 'text-orange-600' : 'text-green-700'}`}>
                                {selectedRoute.proximityAlert?.active ? 'Conductor Llegando' : 'Señal GPS Activa'}
                            </p>
                        </div>
                    )}
                </div>
                        ...
                        {rutasVisibles.map((ruta) => {
                            ...
                            return (
                                <div key={ruta.id} onClick={() => setSelectedRoute(ruta)} className={`border rounded-lg p-3 transition shadow-sm cursor-pointer relative overflow-hidden ${selectedRoute?.id === ruta.id ? 'border-blue-500 ring-1 ring-blue-500 bg-blue-50/10' : 'bg-white border-slate-200 hover:shadow-md'} ${ruta.proximityAlert?.active ? 'border-orange-400 ring-1 ring-orange-400 bg-orange-50/30' : ''}`}>
                                    ...
                                    {ruta.proximityAlert?.active && (
                                        <div className="absolute top-0 left-0 right-0 bg-orange-500 text-white text-[10px] font-black text-center py-1 flex items-center justify-center gap-1 animate-pulse">
                                            <BellRing className="w-3 h-3"/> ¡LLEGANDO A: {ruta.proximityAlert.passenger.toUpperCase()}!
                                        </div>
                                    )}
                                    ...
                                    <div className="flex gap-2 mb-3">
                                        ...
                                        {/* NUEVO: BOTÓN PARA ABRIR CHAT / EVIDENCIAS DESDE LA LISTA */}
                                        {(ruta.status === 'En Ruta' || hasChatOrEvidence) && (
                                            <button onClick={(e) => { e.stopPropagation(); setChatModalRoute(ruta); }} className={`flex flex-col items-center justify-center px-3 rounded border transition-colors ${hasChatOrEvidence ? 'bg-blue-100 text-blue-700 border-blue-200 hover:bg-blue-200' : 'bg-slate-50 text-slate-400 border-slate-200 hover:bg-slate-100'} relative`}>
                                                ...
                                                {ruta.chat && ruta.chat.length > 0 && ruta.chat[ruta.chat.length-1].sender !== 'Despacho' && ruta.chat[ruta.chat.length-1].sender !== 'Sistema' && (
      ...
      {chatModalRoute && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-[fadeIn_0.2s_ease-out]">
              <div className="bg-white w-full max-w-4xl h-[85vh] rounded-2xl shadow-2xl flex overflow-hidden border border-slate-300">
                  ...
                  <div className="w-1/2 flex flex-col border-r border-slate-200 bg-slate-50">
                      ...
                      <div className="p-3 bg-white border-t border-slate-200 flex items-center gap-2 shrink-0">
                          <input 
                              ...
                              placeholder="Escribe al conductor o cliente..." 
                              className="flex-1 bg-slate-100 border border-slate-200 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-blue-500 focus:bg-white transition-colors"
                          />
                  ...
                  {/* COLUMNA DERECHA: EVIDENCIAS Y DETALLES */}
                      ...
                      <div className="flex-1 overflow-y-auto p-6 bg-slate-50 space-y-6">
                          {(!chatModalRoute.evidencias || chatModalRoute.evidencias.length === 0) ? (
                              <div className="text-center py-20 text-slate-400">
                                  ...
                                  <p className="text-xs font-bold uppercase">No hay reportes de ausencia en esta ruta.</p>
                              </div>
                          ) : (
                              chatModalRoute.evidencias.map((ev, idx) => (
                                  <div key={idx} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                                      <div className="bg-red-50 p-3 border-b border-red-100 flex justify-between items-center">
                                          <div>
                                              <p className="text-[10px] font-black text-red-600 uppercase tracking-widest">Reporte de Ausencia</p>
                                              <p className="text-sm font-bold text-slate-800">{ev.passenger}</p>
                                          </div>
                                          <div className="text-right">
                                              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Hora</p>
                                              <p className="text-sm font-mono font-bold text-slate-700">{ev.time}</p>
                                          </div>
                                      </div>
                                      <div className="p-3">
                                          <p className="text-xs text-slate-500 font-medium mb-3 flex items-start gap-1.5"><MapPin className="w-3.5 h-3.5 mt-0.5 shrink-0 text-red-400"/> {ev.address}</p>
                                          {ev.photo ? (
                                              <div className="rounded-lg overflow-hidden border border-slate-200 bg-slate-100 relative group cursor-pointer" onClick={() => window.open(ev.photo, '_blank')}>
                                                  <img src={ev.photo} alt="Evidencia" className="w-full h-48 object-cover" />
                                                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"><p className="text-white text-xs font-bold uppercase tracking-widest">Ver Completa</p></div>
                                              </div>
                                          ) : (
                                              <div className="w-full h-24 bg-slate-100 rounded-lg flex items-center justify-center text-slate-400 text-xs font-bold border border-slate-200">SIN FOTO</div>
                                          )}
                                      </div>
                                  </div>
                              ))
